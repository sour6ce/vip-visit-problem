...
class RandomProblemInput(ProblemInput):
    def __init__(self,n:int, m:int, q:int, min_w:float=1.0, max_w:float=None, seed:int=None):
        ...
        self.ngraph=[set(range(n)) for _ in range(n)]
        self.not_full=set(range(n)) # Nodos que permiten más aristas conectadas a ellos
